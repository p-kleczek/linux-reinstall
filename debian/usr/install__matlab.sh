...
if [ ! -d "$MATLAB_APP_DIR" ]; then
    mkdir -p $MATLAB_TEMP
    unzip -o $MATLAB_ZIP -d $MATLAB_TEMP
    sudo sh "$MATLAB_TEMP/install"

    # Aby zmiany ścieżek były trwałe (pomiędzy uruchomieniami MATLAB-a), należy nadać użytkownikowi uprawienia do ich modyfikacji.
    sudo chown $USERNAME -R "$MATLAB_APP_DIR/toolbox/local/pathdef.m"
    sudo chmod -R +w "$MATLAB_APP_DIR/toolbox/local/pathdef.m"
